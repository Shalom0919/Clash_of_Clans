...
void ClanPanel::setupMemberUI()
{
    ...
    _memberList->setContentSize(Size(560, 220)); // 🆕 减小高度
    _memberList->setPosition(Vec2(-280, -110));  // 🆕 调整位置
    ...
    refreshBtn->setPosition(Vec2(0, -145)); // 🆕 调整位置
...
void ClanPanel::showClanListDialog()
{
    ...
    // ✅ 填充函数
    auto fillList = [this, list](const std::vector<ClanInfoClient>& clans) {
        list->removeAllItems();
