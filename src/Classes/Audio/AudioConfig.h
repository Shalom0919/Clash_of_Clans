...
class AudioConfig {
 ...
 private:
  ...
  /// 音乐ID到路径的映射（使用 EnumClassHash 以兼容旧版 NDK）
  std::unordered_map<MusicId, std::string, EnumClassHash> music_paths_;

  /// 音效ID到路径列表的映射（支持多个变体随机播放）
  std::unordered_map<SoundEffectId, std::vector<std::string>, EnumClassHash> effect_paths_;
};
