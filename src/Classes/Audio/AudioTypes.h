 ...
 * @brief 枚举类哈希函数对象
 *
 * 用于在 std::unordered_map 中使用 enum class 作为键。
 * 某些 NDK 版本（如 r19）不会自动为 enum class 生成哈希函数，
 * 需要手动提供此哈希器。
 *
 * 使用示例：
 * @code
 * std::unordered_map<MusicId, std::string, EnumClassHash> music_map;
 * @endcode
 */
struct EnumClassHash {
  /**
